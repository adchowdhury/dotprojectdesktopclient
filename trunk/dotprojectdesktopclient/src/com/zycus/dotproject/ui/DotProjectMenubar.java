...
//	private JMenuItem	userMenu		= null;
	...
	private void initComponents() {
		...
		/*
		 * add(editMenu = MenuUtility.getMenu("Edit")); editMenu.add(cut =
		 * MenuUtility.getMenuItem("Cut", null)); editMenu.add(copy =
		 * MenuUtility.getMenuItem("Copy", null)); editMenu.add(paste =
		 * MenuUtility.getMenuItem("Paste", null)); editMenu.add(delete =
		 * MenuUtility.getMenuItem("Delete", null));
		 */
		...
		if (ApplicationContext.getCurrentUser().getUserType() == UserType.Administrator || ApplicationContext.getCurrentUser().getUserType() == UserType.CEO
				|| ApplicationContext.getCurrentUser().getUserType() == UserType.Director) {
			//manageMenu.add(companyMenu = MenuUtility.getMenuItem("Company", null));
			// manageMenu.add(divisionMenu = MenuUtility.getMenuItem("Division",
			// null));

		}
		manageMenu.add(projectMenu = MenuUtility.getMenuItem("Project", new DotProjectActionListener.ProjectManagementAction()));
		projectMenu.setIcon(IconHelper.getProjectIcon());
		
		// manageMenu.add(forumMenu = MenuUtility.getMenuItem("Forum", null));
		//manageMenu.add(userMenu = MenuUtility.getMenuItem("User", new DotProjectActionListener.UserManagementAction()));
		...
		settingsMenu.setIcon(IconHelper.getSettingsIcon());
		toolsMenu.add(reportsMenu = MenuUtility.getMenuItem("Reports", new DotProjectActionListener.ReportAction()));
		reportsMenu.setIcon(IconHelper.getReportIcon());
