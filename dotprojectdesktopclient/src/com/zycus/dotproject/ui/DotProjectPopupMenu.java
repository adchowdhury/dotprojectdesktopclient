...
public class DotProjectPopupMenu extends JPopupMenu {
	...
	private JMenuItem				mnuDelete		= null;
	...
	public void show(Component invoker, int x, int y) {
		...
		mnuAddChild.setVisible(selectedIndex >= 0 && 
				ApplicationContext.getViewTYpe() == ViewType.TreeView && 
				canAddTask(selectedObject));
		mnuProperties.setVisible(selectedIndex > 0 && selectedObject instanceof BOTask);
		mnuAddTaskLog.setVisible(selectedIndex > 0 && (selectedObject instanceof BOTask) && 
				((BOTask) selectedObject).getChildTasks().size() <= 0 && 
				((BOTask) selectedObject).canAddTaskLog(ApplicationContext.getCurrentUser()));
		
		mnuDelete.setVisible(selectedIndex > 0 && (selectedObject instanceof BOTask) && 
				((BOTask) selectedObject).canBeDeleted(ApplicationContext.getCurrentUser()));
	...
	private void initComponents(ProjectTaskArea projectTaskArea) {
		...
		add(mnuDelete = MenuUtility.getMenuItem("Delete task", new ProjectTaskArea.TaskDeleteAction(projectTaskArea)));
		...
		mnuAddChild.setIcon(IconHelper.getNewIcon());
		mnuAddTaskLog.setIcon(IconHelper.getNewIcon());
		mnuExpandAll.setIcon(IconHelper.getExpandIcon());
		mnuCollapseAll.setIcon(IconHelper.getCollapseIcon());
		mnuProperties.setIcon(IconHelper.getPropertiesIcon());
		mnuDelete.setIcon(IconHelper.getDeleteIcon());
	}
}